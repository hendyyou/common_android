...
import com.digione.zgb2b.bean.product.ScreenBean;
 ...
 */
public class ProductPhoneScreenFragment extends CommonBaseFragment {
	private ArrayList<ScreenBean> screenList;
	private TableLayout tlLayout;
	private CustomerJsonHttpResponseHandler<ArrayList<ScreenBean>> jsonHttpResponseHandler;
	...
	private void initData() {
		...
		for (int i = 0; i < screenList.size(); i++) {
			...
			tView.setText(screenList.get(i).getScreenSizeRangeName());
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		jsonHttpResponseHandler = new CustomerJsonHttpResponseHandler<ArrayList<ScreenBean>>(activity, true) {

			@Override
			public void processCallSuccess(ArrayList<ScreenBean> outBean, String msg) {
				if (null != outBean) {
					screenList = outBean;
					if (!ZGApplication.getInstance().isSaveScreenData()) {
						...
						persistentService.saveBySharePreference(activity, Constants.DataFile.COMMON_INFO,
								Constants.DataFile.CommonInfoKey.SCREEN_DATA, getEntityJsonString());
						ZGApplication.getInstance().setSaveScreenData(true);
					}
					initData();
				} else {
		...
		titleTextView.setText(R.string.screen_period);

		if (ZGApplication.getInstance().isSaveScreenData()) {
			PersistentService persistentService = new PersistentServiceImpl();
			String dataString = persistentService.getStringBySharePreference(activity, Constants.DataFile.COMMON_INFO,
					Constants.DataFile.CommonInfoKey.SCREEN_DATA);
			Gson gson = new Gson();
			Type listType = new TypeToken<ArrayList<ScreenBean>>() {
			}.getType();
			screenList = gson.fromJson(dataString, listType);
			initData();
		} else {
			// 请求网络,获取用户信息
			HttpClient userClient = HttpClient.getInstall(activity);
			String url = Constants.Url.LOAD_SCREEN_SIZE_RANGE_018;
	...
	class ProductkListener implements OnClickListener {
		...
		public void onClick(View v) {
			ScreenBean bean = (ScreenBean) v.getTag();
			Bundle bundle = new Bundle();
			bundle.putString(ArgName.ProductList.TITLE, getString(R.string.phone) + "(" + bean.getScreenSizeRangeName()
					+ ")");
			bundle.putString(ArgName.ProductList.URL, bean.getScreenSizeRangeSearchUrl()); // 请求的URL
