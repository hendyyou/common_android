...
import com.digione.zgb2b.bean.product.PriceBean;
 ...
 */
public class ProductPhonePriceFragment extends CommonBaseFragment {
	private ArrayList<PriceBean> priceList;
	private TableLayout tlLayout;
	private CustomerJsonHttpResponseHandler<ArrayList<PriceBean>> jsonHttpResponseHandler;
	...
	private void initData() {
		...
		for (int i = 0; i < priceList.size(); i++) {
			...
			tView.setText(priceList.get(i).getAdvicePriceRangeName());
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		jsonHttpResponseHandler = new CustomerJsonHttpResponseHandler<ArrayList<PriceBean>>(activity, true) {

			@Override
			public void processCallSuccess(ArrayList<PriceBean> outBean, String msg) {
				if (null != outBean) {
					priceList = outBean;
					if (!ZGApplication.getInstance().isSavePriceData()) {
						...
						persistentService.saveBySharePreference(activity, Constants.DataFile.COMMON_INFO,
								Constants.DataFile.CommonInfoKey.PRICE_DATA, getEntityJsonString());
						ZGApplication.getInstance().setSavePriceData(true);
					}
					initData();
				} else {
		...
		titleTextView.setText(R.string.price_period);

		if (ZGApplication.getInstance().isSavePriceData()) {
			PersistentService persistentService = new PersistentServiceImpl();
			String dataString = persistentService.getStringBySharePreference(activity, Constants.DataFile.COMMON_INFO,
					Constants.DataFile.CommonInfoKey.PRICE_DATA);
			Gson gson = new Gson();
			Type listType = new TypeToken<ArrayList<PriceBean>>() {
			}.getType();
			priceList = gson.fromJson(dataString, listType);
			initData();
		} else {
			// 请求网络,获取用户信息
			HttpClient userClient = HttpClient.getInstall(activity);
			String url = Constants.Url.LOAD_ADVICE_PRICE_RANGE_019;
	...
	class ProductkListener implements OnClickListener {
		...
		public void onClick(View v) {
			PriceBean bean = (PriceBean) v.getTag();
			Bundle bundle = new Bundle();
			bundle.putString(ArgName.ProductList.TITLE, getString(R.string.phone) + "(" + bean.getAdvicePriceRangeName()
					+ ")");
			bundle.putString(ArgName.ProductList.URL, bean.getAdvicePriceRangeSearchUrl()); // 请求的URL
